...
class DangleSpider(BaseSpider):
	name = 'DangleSpider'
	allowed_domains = ['www.d.cn']
	...
	def __init__(self, mode, *args, **kwargs):
		...
		if self.mode == 'list':
			self.start_urls.append('http://android.d.cn/software/1/34/-1')
		elif self.mode == 'meta':
			client = MongoClient()
	...
	def parse(self,response):
		...
		if self.mode == self.modeList[0]:
			...
			for url in sel.xpath('//p[@class="g-name"]/a/@href').extract():
				listItem = ListItem()
				listItem['mode'] = self.mode
				listItem['market'] = 'dangle'
				listItem['url'] = url
				yield listItem
			...
			metaItem['market'] = 'dangle'
			metaItem['url'] = response.url
			metaItem['title'] = sel.xpath('//div[@class="de-head-l"]/h1/text()').extract()
			metaItem['version'] = ''
			metaItem['desc'] = sel.xpath('//div[@class="de-intro-inner"]/text()').extract()
			...
			if self.inited == False:
				...
				for oneItem in downItems.find({}, {'dangle':1, '_id':0}):
					for oneId in oneItem['dangle']:
				...
				downItem['url'] = sel.xpath('//a[@class="de-head-btn de-pc-btn"]/@href').extract()
