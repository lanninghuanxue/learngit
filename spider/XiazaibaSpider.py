...
class XiazaibaSpider(BaseSpider):
	name = 'XiazaibaSpider'
	...
	def __init__(self, mode, *args, **kwargs):
		...
		if self.mode == 'list':
			self.start_urls.append('http://www.xiazaiba.com/downlist/android_129.html')
		elif self.mode == 'meta':
			client = MongoClient()
	...
	def parse(self,response):
		...
		if self.mode == self.modeList[0]:
			...
			for url in sel.xpath('//a[@class="name"]/@href').extract():
				listItem = ListItem()
			...
			if self.inited == False:
				...
				downItem['url'] = sel.xpath('//div[@class="app-dl"]/a[@class="btn"]/@href').extract()
